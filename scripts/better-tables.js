...
export class BetterTables {
	...
	async createTableFromCompendium(tableName, compendiumName, { weightPredicate = null } = {}) {
		...
		ui.notifications.info(MODULE.ns + ` | Starting generation of rolltable for ${compendium.title} (${compendiumName}) with ${compendiumSize} entries.`);
		compendium
			.getDocuments()
	...
	async updateSpellCache(pack) {
		if (game.user.isGM) {
			...
			if ((!pack && spellCompendium) || pack === defaultPack) {
				const spellCompendiumIndex = await spellCompendium.getIndex({ fields: ['system.level', 'img'] });
	...
	static async _renderMessage(message) {
		const cardHtml = await renderTemplate('modules/better-rolltables/templates/loot-chat-card.hbs', message.flags.betterTables.loot);
    ...
    */
	...
	static async _addButtonsToMessage(message, html) {
		...
		if (game.settings.get(MODULE.ns, BRTCONFIG.SHOW_REROLL_BUTTONS)) {
			// reroll button
			const rerollButton = $(`<a class="roll-table-reroll-button" title="${game.i18n.localize('BRT.DrawReroll')}">`).append(
		...
		if (
			...
			const currencyShareButton = $(`<a class="roll-table-share-currencies" title="${game.i18n.localize('BRT.Currency.Buttons.Share.Label')}">`).append(
		...
		if (game.settings.get(MODULE.ns, BRTCONFIG.SHOW_OPEN_BUTTONS)) {
			...
			if (document) {
				const openLink = $(`<a class="roll-table-open-table" title="${game.i18n.localize('BRT.OpenRolltable')}">`).append(
	...
	static async _shareCurrenciesToPlayers(message, html) {
		...
		const newMessage = await BetterTables._renderMessage(mergeObject(message, { 'flags.betterTables.loot.shared': true }));
		...
		if (game.settings.get(MODULE.ns, BRTCONFIG.ROLL_TABLE_FROM_JOURNAL)) {
			...
			$(html)
				...
				.each((index, link) => {
					...
					const rollNode = $(
						`<a class="roll-table-roll-link" title="${game.i18n.localize('BRT.DrawReroll')}"><i class="fas fa-dice-d20"></i></a>`
					).click(async () => {
