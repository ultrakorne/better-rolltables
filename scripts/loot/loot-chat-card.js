...
export class LootChatCard {
	...
	async prepareCharCart(table) {
		...
		if (this.numberOfDraws > 1) {
			flavorString = game.i18n.format('BRT.DrawResultPlural', { amount: this.numberOfDraws, name: table.name });
		} else if (this.numberOfDraws > 0) {
			flavorString = game.i18n.format('BRT.DrawResultSingular', { amount: this.numberOfDraws, name: table.name });
		} else {
			flavorString = game.i18n.format('BRT.DrawResultZero', { name: table.name });
		}
