...
export class StoryBuilder {
  ...
  async drawStory() {
    ...
    for (const entry of draw.results) {
      ...
      if (entry.type === 1 && entry.documentCollection === 'JournalEntry') {
        ...
        const entity = await Utils.findInCompendiumByName(
        ...
        if (!entity) {
          ...
          errorString =
            'Only Journal entries are supported in the story generation as table results';
        }
      } else {
        errorString =
          'Only Journal entries are supported in the story generation as table results';
    ...
    if (tableMatch) {
      ...
      if (tableResult.type !== 0) {
        ui.notifications.warn(
          `only text result from table are supported at the moment, check table ${table.name}`
        );
      }
  ...
  _generateStory(story) {
    ...
    while ((matches = regex.exec(story)) != null) {
      ...
      if (!value) {
        ui.notifications.error(
          `cannot find a value for token ${matches[1]} in #story definition`
        );
        continue;
      }
